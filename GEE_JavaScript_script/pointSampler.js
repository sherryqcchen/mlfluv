exports.sampleFromPoint = function(coordinates, year) {
    ...
    // var years = ee.List.sequence(2017, 2023, 1);
    ...
    // Get the first image date (system: time_start) with max unmasked_percentage.
    ...
    // var s2_visparam = {bands:['B4','B3','B2'], min:0, max:6000};
    // Map.addLayer(clear_s2, s2_visparam, 'Clear S2 image of the clear date');
    
    // Get S2 image from a cloudy date within a month after the chosen clear date.
    var cloudy_s2 = s2Col.filter(ee.Filter.or(
                                ee.Filter.date(clear_date.advance(1, 'day'), clear_date.advance(30, 'day')),
                                ee.Filter.date(clear_date.advance(-30, 'day'), clear_date.advance(-1, 'day'))))
    ...
    // Map.addLayer(cloudy_s2, s2_visparam, 'Cloudy S2 image of the cloudy date');
    
    // // // -------------------Fethcing and preprocessing Sentinel-1 images--------------------//
    // Get preprocessed S1 image of clear date
    var clear_end = clear_date.advance(2, 'week');
    var cloudy_end = cloudy_date.advance(2, 'week');
    
    var clear_s1_col = getS1Preprocess(clear_date, clear_end, AOI)
    ...
    var cloudy_s1_col = getS1Preprocess(cloudy_date, cloudy_end, AOI);
    ...
    var clear_s1_view = clear_s1_col.map(helper.add_ratio_lin).map(helper.lin_to_db2);
    var cloudy_s1_view = cloudy_s1_col.map(helper.add_ratio_lin).map(helper.lin_to_db2);
    var s1_visparam = {bands:['VV','VH','VVVH_ratio'],min: [-20, -25, 1],max: [0, -5, 15]};
    print(clear_s1_view.first())
    Map.addLayer(clear_s1_view, s1_visparam, 'S1 from a clear date db view');
    ...
    var rainfall = ee.ImageCollection('UCSB-CHG/CHIRPS/DAILY')
                      ...
                      .filter(ee.Filter.or(
                              ee.Filter.date(clear_date, cloudy_date),
                              ee.Filter.date(cloudy_date, clear_date)))
    ...
    print(cloudy_date)
    print(clear_s1_date)
    print(cloudy_s1_date)
    
    var para_feature_col = ee.FeatureCollection([ee.Feature(AOI, dict)]);
    
    var lng = ee.Number(coords.get(0)).multiply(ee.Number(100)).abs().format('%.0f');
    var lat = ee.Number(coords.get(1)).multiply(ee.Number(100)).abs().format('%.0f');
    
    var pointID = ee.String(lng.cat('_').cat(lat).cat('_')).getInfo()
    // print(pointID)
    ...
    Export.table.toDrive({
      ...
      folder: "MLFluvData_DWS12_"+pointID+system_index,
      fileFormat: 'CSV'
    });
    ...
    Export.image.toDrive({
      image: clear_s1_view.select('VV', 'VH', 'VVVH_ratio'),
