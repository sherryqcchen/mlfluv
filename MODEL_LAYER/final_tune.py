...
if __name__ == "__main__":
    ...
    if config_params["model"].get("class_weights") is not None:
        print("Ignoring model.class_weights; calculating weights from this run's final-tune training set.")
    weight_suffix = safe_weight_suffix(which_label, "final", log_num, tune_mode, "train", train_fold)
    print(f"Calculating class weights from this final-tune training set with suffix {weight_suffix}.")
    ...
    for i, (image, mask) in enumerate(test_loader):
        ...
        plot_inference_result(s2_false_color, s1_vv, y, y_pred_map, os.path.join(output_folder,'preds'), i)
