# this script is for directly tune inital trained model on hand labels. 
# It is used for comparing with the result of models after incremental learning
import argparse
import cv2
import shutil
import os
...
if __name__ == "__main__":
    ...
    shutil.copy(os.path.join(root_path, f'script/MODEL_LAYER/direct_tune.py'), os.path.join(output_folder, 'direct_tune.py'))
    ...
    if with_extra_urban:
        fold_data_path = os.path.join(config_params['data_loader']['train_paths'], f'final_test_{which_label}_4_fold')
    else:
        fold_data_path = os.path.join(config_params['data_loader']['train_paths'], f'final_test_{which_label}_fold')
